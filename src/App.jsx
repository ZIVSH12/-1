          ...
          <div>
            ...
            <ul className="mt-6 grid gap-2 text-white/80">
              {["חשיבה אסטרטגית למותג", "UI/UX שממיר לידים", "ביצוע פיקסל־פרפקט"].map((t) => (
                <li key={t} className="flex items-center gap-2">
                  <Check className="size-5 text-pink-400" /> {t}
                </li>
              ))}
            </ul>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a href={`tel:${BRAND.phone}`} className="group inline-flex items-center gap-2 rounded-2xl bg-white text-black px-5 py-3">
                התקשרו עכשיו — {fmt(BRAND.phone)}
                <ArrowRight className="size-4 transition-transform group-hover:-translate-x-0.5" />
              </a>
              <a href="#work" className="inline-flex items-center gap-2 rounded-2xl border border-white/20 px-5 py-3 hover:bg-white/5">
                צפו בעבודות
              </a>
            </div>
            ...
            <div className="mt-10 grid grid-cols-3 gap-4 text-center text-sm">
              {[
                ...
                <GlassCard key={s.t} className="py-4">
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {items.map((s) => (
            <GlassCard key={s.title} className="p-6 hover:bg-white/[0.05] transition">
              <div className="mb-3 inline-flex size-12 items-center justify-center rounded-2xl bg-gradient-to-br from-pink-500 to-rose-400 text-white">
          ...
          {works.map((w,i)=>(
            <GlassCard key={i} className="overflow-hidden group">
              <img
                src={w.img}
                alt={w.title}
                className="aspect-[4/3] w-full object-cover transition duration-300 group-hover:scale-[1.02] opacity-90"
                onError={(e)=>{ e.currentTarget.style.display='none'; }}
              />
          ...
          <div className="grid items-center gap-6 lg:grid-cols-2">
            <div>
              <h3 className="text-2xl sm:text-3xl font-semibold leading-tight">
                מוכנים למיתוג שנראה כמו מיליון דולר?
              </h3>
              <p className="mt-2 text-white/70">
                דף נחיתה מהיר, זהות מותג חדה, וחומרים שמוכנים לפרסום.
              </p>
            </div>
            <div className="flex flex-wrap gap-3 justify-start lg:justify-end">
              <a href={`tel:${BRAND.phone}`} className="rounded-2xl bg-white text-black px-5 py-3 inline-flex items-center gap-2">
                התקשרו — {fmt(BRAND.phone)} <Phone className="size-4" />
              </a>
              <a href={`mailto:${BRAND.email}`} className="rounded-2xl border border-white/20 px-5 py-3 inline-flex items-center gap-2 hover:bg-white/5">
                אימייל <Mail className="size-4" />
              </a>
            </div>
          </div>
        </GlassCard>
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-2">
          ...
          <GlassCard className="p-6">
            <form className="grid gap-4" onSubmit={(e)=>e.preventDefault()}>
              ...
              <button className="rounded-2xl bg-white text-black px-5 py-3 inline-flex items-center gap-2 justify-center">
...
function Field({ label, type="text", placeholder="", isTextarea=false }) {
  ...
  return (
    <div className="grid gap-2">
      ...
      {isTextarea ? (
        <textarea rows={4} className={cls} placeholder={placeholder} />
      ) : (
...
function CallFloating() {
  return (
    <a
      ...
      className="fixed bottom-5 left-5 z-50 inline-flex items-center gap-2 rounded-full bg-pink-500 px-4 py-2 shadow-xl hover:scale-[1.02] transition"
